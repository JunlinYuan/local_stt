...
public final class ReplacementManager: Sendable {
    ...
    public init(bundledFileURL: URL? = nil, directoryURL: URL? = nil) {
        ...
        // Merge bundled replacements into working copy on every launch.
        // This picks up new rules added on macOS without losing iOS-only rules.
        if let bundled = bundledFileURL,
           FileManager.default.fileExists(atPath: bundled.path) {
            mergeFromBundle(bundled)
        }
    }
    ...
    /// Merge rules from the bundled file into the working copy (additive).
    /// New rules (by `from` field, case-insensitive) are appended; existing rules are preserved.
    private func mergeFromBundle(_ bundledURL: URL) {
        guard let data = try? Data(contentsOf: bundledURL),
              let file = try? JSONDecoder().decode(FileFormat.self, from: data)
        else { return }

        let currentRules = _rules.value
        let existingFroms = Set(currentRules.map { $0.from.lowercased() })

        var merged = currentRules
        for rule in file.replacements {
            if !existingFroms.contains(rule.from.lowercased()) && merged.count < Self.maxRules {
                merged.append(rule)
            }
        }

        if merged.count != currentRules.count {
            _rules.set(merged)
            saveToFile()
        }
    }
