#!/usr/bin/env python3
"""End-to-end tests for the Gemma 4 local STT provider.

Tests the gemma4_stt module against real audio files:
  - /tmp/stt_test_23.wav (French ~11s)
  - /tmp/stt_test_24.wav (English ~11s)
  - /tmp/stt_test_25.wav (Chinese ~11s)

Run: python3 tests/test_gemma4_provider.py
"""
...
# ── Test 1: is_gemma4_available ──────────────────────────────────────────────

print("\n=== Test 1: is_gemma4_available() ===")
avail = gemma4_stt.is_gemma4_available()
report("mlx-vlm installed", avail, f"is_gemma4_available() = {avail}")

if not avail:
    print("\nFATAL: mlx-vlm not installed — cannot continue.")
    print("Install with: cd backend && uv sync --extra local-gemma")
    sys.exit(1)

# ── Test 2: Singleton creation ───────────────────────────────────────────────

print("\n=== Test 2: get_gemma4_stt() ===")
try:
    stt = gemma4_stt.get_gemma4_stt()
...
for path, lang, label in TEST_FILES:
    ...
    try:
        ...
        # Provider check
        report(f"{label} provider=='local'", result.get("provider") == "local",
        ...
        print(f"  TEXT: {text}")
        print(f"  TIMING: inference={pt:.3f}s  wall={wall_time:.3f}s  audio={dur:.1f}s")
...
gemma4_stt._gemma4_stt = None
stt_no_vocab = gemma4_stt.get_gemma4_stt()
...
print("\n=== Test 5: Error handling — empty/tiny bytes ===")
try:
    result_empty = stt_no_vocab.transcribe(b"", language="en")
    # Should return empty text (too short)
...
gemma4_stt._gemma4_stt = None
stt_auto = gemma4_stt.get_gemma4_stt()
